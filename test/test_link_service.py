...
def test_shorten_url(link_service):
    original_url = 'https://example.com'
    link = link_service.shorten_url(original_url)

    assert link is not None
    assert link.original_url == original_url
    assert len(link.shortened_url) == 6

def test_get_original_url(link_service):
    original_url = 'https://example.com'
    link = link_service.shorten_url(original_url)

    fetched_link = link_service.get_original_url(link.shortened_url)
