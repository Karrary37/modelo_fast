...
class SQLAlchemyLinkRepository(LinkRepository):
    ...
    def save_link(self, link: LinkModel) -> LinkModel:
        db_link = LinkModel(id=link.id, original_url=str(link.original_url), shortened_url=link.shortened_url)
        self.db.add(db_link)
        self.db.commit()
    ...
    def get_link_by_shortened_url(self, shortened_url: str) -> Optional[LinkModel]:
        return self.db.query(LinkModel).filter(LinkModel.shortened_url == shortened_url).first()
